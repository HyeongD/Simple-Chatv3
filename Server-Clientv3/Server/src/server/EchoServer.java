...
public class EchoServer extends AbstractServer {
    ...
    static final String PASSWORDFILE = "C:\\2022-GKES\\Server-Clientv3\\Server\\src\\passwords.txt";
    ...
    public void sendToAllClients(Object msg) {
        ...
        for (int i = 0; i < clients.length; i++) {
            ...
            try {
                ...
                serverUI.display("WARNING -Cannot send message to client.");
                e.printStackTrace();
            }
        }
    }
    ...
    public synchronized void handleMessageFromServerUI(String message) {
        ...
        if (message.startsWith("#?") || message.startsWith("#help")) {
            serverUI.display("\nServer-side command list: "
                    ...
                    + "n#unblock <loginID> -- Unblock messages from the specified client."
                    + "\n#warn <loginID> -- Sends a warning message to the specofoed client."
        ...
        if (!(message.startsWith("#"))) {
            ...
            sendChannelMessage("SERVFER MESSAGE> " + message, (serverChannel == null ? "main" : serverChannel),
                    "server");
        } else {
    ...
    public synchronized void handleMessageFromClient(Object msg, ConnectionToClient client) {
        ...
        if (((Boolean) (client.getInfo("passwordVerified"))).booleanValue()) {
            ...
            if (!command.startsWith("#")) {
                sendChannelMessage(client.getInfo("loginID") + ", " + command, (String) client.getInfo("channel"),
                    ...
                    if (isLoginBeingUsed(userToBlock, false) && !userToBlock.equals("server")) {
                        ...
                        if (((String) (toBlock.getInfo("fwdClient"))).equals(((String) (client.getInfo("loginID"))))) {
                            ...
                            try {
                                ...
                                client.sendToClient(
                                        "Forwarding from " + toBlock.getInfo("loginID")
        ...
        try {
            ...
            if (loginID.toLowerCase().equals("server")) {
                if (!blockedUsers.contains(sender)) {
                    ...
                    try {
                        client.sendToClient(
                                "Cannot send message because " + loginID + " is blocking messages from you");
                    } catch (IOException e) {
                        serverUI.display("Warning: Error sending message.");
                        e.printStackTrace();
                    }
                    ...
                    for (int i = 0; i < clients.length; i++) {
                        ...
                        if (c.getInfo("loginID").equals(loginID)) {
                            if (!(((Vector) (c.getInfo("blockedUsers"))).contains(sender))) {
                                ...
                                serverUI.display("PRIVATE MESSAGE from \"" + message + "\"from " + sender + " to "
                                        + c.getInfo("loginID"));
                            } else {
                                sendToClientOrServer(client,
                                        "Cannot send message because " + loginID + " is blocking message from you");
                            }
                        }
                    }
                } catch (IOException e) {
    ...
    private void checkForBlocks(String login, ConnectionToClient client) {
        ...
        if (results.equals("User block check: ")) {
            results += "\nNo user is blockiing messages from you.";
        }
    ...
    private void clientLoggingIn(String message, ConnectionToClient client) {
        ...
        if ((client.getInfo("loginID").equals("")) && (message.equals("guest"))) {
            ...
            // String val = client.getInfo("creatingNewAccount").toString();
            if ((client.getInfo("loginID").equals("")) && message != null) {

                client.setInfo("loginID", message);
                // System.out.println(client.getInfo("loginID"));
                try {
                    client.sendToClient("Enter a new  password :");
                ...
                if ((!client.getInfo("loginID").equals("")) && client.getInfo("password").equals("")
                        &&
                        ((boolean) (client.getInfo("creatingNewAccount")) == true)) {
                    client.setInfo("password", message);
                    if (!isLoginUsed((String) (client.getInfo("loginID")))) {
                        ...
                        sendToAllClients(client.getInfo("loginID") + " has bob logged on.");
                    } else {
                    ...
                    if (client.getInfo("loginID").equals("")) {
                        ...
                        // System.out.println(message);
                        // addClientToRegistry((client.getInfo("loginID").toString()), message);
    ...
    private void addClientToRegistry(String clientLoginID, String clientPassword) {
        try {
            ...
            outputFile.close();
            // serverUI.display("\nloginID and password saved successfully\n");
            /*
    ...
    private boolean isValidPwd(String loginID, String password, boolean verifyPassword) {
        try { // jkkd
            try (FileInputStream inputFile = new FileInputStream(PASSWORDFILE)) {
                boolean eoln = false;
                boolean eof = false;
                while (!eof) {
                    eoln = false;
                        ...
                        if (character == -1) {
                            ...
                            if (character == LINEBREAK) {
                                ...
                                if ((str.substring(0, str.indexOf(" ")).equals(loginID))
                                        && ((str.substring(str.indexOf(" ") + 1).equals(password))
                                                || (!verifyPassword))) {
                                    return true;
                                }
                            } else {
    ...
    public void handleDisconnect(ConnectionToClient client) {
        if (!client.getInfo("loginID").equals("")) {
            try {
                Thread[] clients = getClientConnections();
                for (int i = 0; i < clients.length; i++) {
                ...
                sendToAllClients(
                        ((client.getInfo("loginID") == null) ? "" : client.getInfo("loginID"))
                                + " has disconnected.");
            } catch (IOException e) {
                serverUI.display("Warning: Error sending message.");
            }
